...
def plot_entropy_metrics(entropies, varentropies, output_text, tokenizer, save_path='entropy_plot.png'):
    ...
    plt.subplot(1, 2, 1)
    plt.bar(range(len(entropies)), entropies, label='Entropy', color='blue', linewidth=2)
    plt.title('Token-wise Entropy', fontsize=14, pad=20)
    plt.xlabel('Token Position', fontsize=12)
    plt.ylabel('Entropy', fontsize=12)
    plt.xticks(range(len(tokens)), tokens, rotation=90, ha='right', fontsize=8)
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=12)
    
    plt.subplot(1, 2, 2)
    plt.bar(range(len(varentropies)), varentropies, label='Varentropy', color='red', linewidth=2)
